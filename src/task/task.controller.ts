...
export class TaskController {
  ...
  async sendToConfirm(@Param('id') id: string, @Body() dto: AddNoteDto, @User() user) {
    ...
    const task = await this.taskService.updateTaskStatus(id, Status.SEND_TO_CONFIRM, user?.login);
  ...
  async complete(@Param('id') id: string, @User() user) {
    const task = await this.taskService.updateTaskStatus(id, Status.DONE, user?.login);
    this.taskService.update(id, { endDate: new Date() }, user.login);
    return task;
  }
  ...
  async rollBack(@Param('id') id: string, @Body() dto: AddNoteDto, @User() user) {
    ...
    const task = await this.taskService.updateTaskStatus(id, Status.ROLLED_BACK, user?.login);
